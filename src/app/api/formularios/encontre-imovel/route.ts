...
export async function GET() {
  try {
    const filePath = 'public/dados/formularios-encontre-imovel.json'
...
export async function POST(request: NextRequest) {
  try {
    const dados: Omit<FormularioEncontreImovel, 'id' | 'createdAt' | 'lido'> = await request.json()
    
    // Buscar dados atuais
    const filePath = 'public/dados/formularios-encontre-imovel.json'
    let currentData: { formularios: FormularioEncontreImovel[] } = { formularios: [] }
    ...
    const novoFormulario: FormularioEncontreImovel = {
      ...dados,
    ...
    await octokit.repos.createOrUpdateFileContents({
      ...
      repo: REPO_NAME,
      path: filePath,
      message: `Formulário: Encontre imóvel de ${dados.nome}`,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if ('content' in response.data) {
      ...
      data.formularios = data.formularios.filter((f: FormularioEncontreImovel) => f.id !== id)
      
      await octokit.repos.createOrUpdateFileContents({
        owner: REPO_OWNER,
        repo: REPO_NAME,
        path: filePath,
        message: `Admin: Remove formulário de encontre imóvel ${id}`,
