...
export default function ImovelDetalhePage() {
  ...
  const [fotosVerticais, setFotosVerticais] = useState<Set<number>>(new Set()) // Índices 1-4 que são muito verticais
  ...
  useEffect(() => {
    const loadImovel = async () => {
      try {
        ...
        setFotosVerticais(new Set()) // Reset ao mudar imóvel
  ...
  // Detectar fotos verticais ao carregar (apenas para as 4 menores)
  const handleImageLoad = (index: number, event: React.SyntheticEvent<HTMLImageElement>) => {
    if (index < 1 || index > 4 || fotosVerticais.has(index)) return
    
    const img = event.currentTarget
    if (!img.naturalWidth || !img.naturalHeight) return
    
    const aspectRatio = img.naturalWidth / img.naturalHeight
    // Se muito vertical (aspect ratio < 0.7), precisa object-contain
    if (aspectRatio < 0.7) {
      setFotosVerticais(prev => new Set(prev).add(index))
    }
  }

  const precisaObjectContain = (index: number) => fotosVerticais.has(index)
